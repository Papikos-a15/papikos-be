...
public class OtherNotificationFactory implements NotificationFactory {

    private final UUID userId;

    public OtherNotificationFactory(UUID userId) {
        this.userId = userId;
    }

    @Override
    public Notification createNotification(String title, String message) {
        return new Notification(UUID.randomUUID(), userId, title, message, LocalDateTime.now(), NotificationType.OTHER, false);
    }
}